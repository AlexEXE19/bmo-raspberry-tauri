...
export default function App() {
  const [screen, setScreen] = useState<string>("Idle");
  ...
  return (
    <div className="h-screen w-screen bg-slate-100 flex items-center justify-center overflow-hidden p-4">
      {/* BMO Device Body */}
      <div className="relative w-full max-w-md h-full max-h-[800px] bg-linear-to-br from-teal-500 to-teal-600 rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.4)] border-8 border-teal-700 flex flex-col p-6">
        {/* BMO Text (vertical on left side) */}
        <div className="absolute left-2 top-20 flex flex-col gap-0">
          <span className="text-teal-900 tracking-widest origin-center -rotate-90 whitespace-nowrap -translate-x-5">
            BMO
          </span>
        </div>
